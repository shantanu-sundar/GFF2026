...
**The desktop section follows the booth, it is no longer independent of it.** Booth
mode was where the Skills terminal was first drawn portrait, and that portrait design
is the one that shipped — so the page adopted it rather than keeping a second, wider
one. The Skills screen is 360x800 on the page too, the terminal and the store rules
are shared rather than `body.booth`-scoped, and everything else here still is.
...
### Agent Skills has no laptop anywhere

The booth had nowhere to put one, so Skills was drawn portrait for the kiosk: title
bar, transcript, the workspace it wrote under that, `# Agent Toolkit — interactive section
...
come out identical, surface for surface.
 ...
 prompt at the foot. That is
now the only Skills design there is. The page shows the same thing, because two
designs for one demo is one more than the section can keep honest.

`setChassis()` adds `.as-term` — **the JS is still not branched** — and on the page
that grows the phone to 382x822, whose screen is exactly the 360x800 the booth
renders. Same numbers, so the terminal, the store and the drop-in are one set of
rules serving both surfaces instead of two sets to keep in sync.

This gives up the one gesture the section made for Skills: the phone visibly
*widening* into a laptop when you tap in. What survives is the grammar — traffic
lights, JetBrains Mono, a `# Agent Toolkit — interactive section
...
come out identical, surface for surface.
 ...
 prompt, `~/ecommerce-app` and `claude code` in the
...
screen inverts the problem — 360 wide but 800 tall, with height to spare — so the store
puts all of it back and lets the grid run down the page the way a phone shop does.
...
**Those rules are no longer booth-scoped, and the `max-width: 700px` block is gone.**
The store and the drop-in only ever run inside the Skills demo, whose screen is 360x800
on both surfaces, so one copy serves both — and a layout keyed to the window width
would only ever be wrong now that the screen no longer tracks it.
...
### The header carries the agent, and nothing else

A Relay run's header is the agent's face and its name. That is all. The right-hand chip used to
read **no-code** on all three, which is a claim aimed at somebody deciding whether to buy the
product — and by the time you are watching a run, that decision is made and the thing is
working in front of you. It also competed with the one job that header has, which is to say at
a glance **which of the three agents you are looking at**.

`chips.right` is optional now. The demos that keep one are the ones where it is telling you
something you cannot see: `11/40 tools` on the Toolkit, `claude code` on Skills.
...
## Agent Skills is a terminal, on a taller phone
...
It used to answer that by swapping the device for a 596×376 laptop lid. The booth killed the
lid — a kiosk is a 360x800 portrait screen with nowhere to widen to — and the portrait
terminal drawn for it is the design that shipped. So the device stays a phone and only the
*renderer* changes. `surface: 'terminal'` on the demo does three things at once:

- **the chassis** — `.phone` gains `.as-term` (382×822, so the screen inside the 11px bezel
  is exactly the booth's 360×800) and the demo card gains `.wide` (a 414px phone column).
  The notch goes, because the terminal brings its own title bar and would sit under it. The
  `.phone-wrap` keeps its `722px` min-height, so the card under the shorter demos never
  reflows; Skills is the one demo that runs past it, by 100px. The size transitions, so the
  phone visibly *grows* when you tap in.
- **the screen** — `.screen.term` re-lays the same four children (`ph-top`, `thread`,
  `ledger`, `composer`) onto a CSS grid, stacked: title bar, transcript, the workspace it
  wrote, `# Agent Toolkit — interactive section
...
a Hindi question would have shown the opposite of the claim.
 ...
 prompt at the foot. Traffic lights, JetBrains Mono, a `# Agent Toolkit — interactive section
...
a Hindi question would have shown the opposite of the claim.
 ...
 prompt — and a
  **light** ground, the same warm liquid-glass paper as the rest of the section. A terminal
  is legible on paper as long as the grammar is intact; a dark slab in this layout read as a
...
over the whole screen and `term.view` says which page it is on:
...
four. Nothing is dropped at any window size any more: the store renders whole, as the phone
storefront described above, because the screen it renders into is a fixed 360x800.
